...
const ZAMBIA = { code: 'ZM', dial: '+260', name: 'Zambia' };
...
export const UnifiedAuthPage: React.FC = () => {
    ...
    const [businessCountry] = useState(ZAMBIA.name);
...
const PinEntry: React.FC<{ label: string; value: string; onChange: (value: string) => void; helper: string }> = ({ label, value, onChange, helper }) => {
    ...
    return <div className="space-y-2"><span className="block text-[10px] font-black uppercase tracking-[0.16em] text-slate-400">{label}</span><div className={`relative grid grid-cols-4 gap-2 rounded-2xl border p-2 transition ${focused ? 'border-amber-300/90 bg-amber-300/[0.07] shadow-[0_0_0_4px_rgba(251,191,36,.10)]' : 'border-white/10 bg-black/20'}`} onClick={() => inputRef.current?.focus()}><input ref={inputRef} type="text" inputMode="numeric" autoComplete="one-time-code" maxLength={4} value={value} onChange={(event) => onChange(event.target.value.replace(/\D/g, '').slice(0, 4))} onFocus={() => setFocused(true)} onBlur={() => setFocused(false)} aria-label={label} className="sr-only" />{[0, 1, 2, 3].map((index) => <div key={index} className={`flex h-12 items-center justify-center rounded-xl border text-2xl font-black transition sm:h-14 ${value[index] ? 'border-amber-300/50 bg-gradient-to-b from-orange-400/20 to-amber-300/10 text-amber-100' : focused && index === value.length ? 'border-amber-300/70 bg-amber-300/10 text-amber-200' : 'border-white/10 bg-white/[0.035] text-slate-600'}`}>{value[index] ?? '·'}</div>)}</div><p className="text-xs leading-relaxed text-slate-500">{helper}</p></div>;
};
